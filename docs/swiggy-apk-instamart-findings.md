...
## 8. Recommendations for the crawler (`tools/pw_catalog.js`, instamart adapter)
...
4. **Probe MxN curated collections** via `campaign-collection/mxn?layoutId=<id>&customerPage=STORES_MxN_*` (Gourmet `12276`/`STORES_MxN_52`, Handpicked `5984`/`STORES_MxN_3`) as additional curated category grids.
5. **Discovery endpoint** `disc.swiggy.com` `evaluatePage` is the source of the home/section widget tree — only relevant if we ever drive the app WebView; not needed for `instamart.in`.
